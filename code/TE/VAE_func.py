...
# 学习参数
parser.add_argument('--epochs', type=int, default=200)#epochs
parser.add_argument('--batch_size', type=int, default=10)#batch_size
parser.add_argument('--lr', type=float, default=0.0003)
parser.add_argument('--activa', type=str, default='selu')
parser.add_argument('--variables', type=int, default=33)
parser.add_argument('--time_steps', type=int, default=6)
parser.add_argument('--filters', type=int, default=10)
parser.add_argument('--kersize', type=int, default=3)
parser.add_argument('--latent_pre', type=int, default=60)
parser.add_argument('--latent', type=int, default=60)
parser.add_argument('--mse_beta', type=int, default=10)  # beta要大于1
parser.add_argument('--beta', type=int, default=1)  # beta要大于1
parser.add_argument('--l1_x', type=float, default=0.0)  # beta要大于1
...
def slice_x(data):
    ...
    data = np.hstack((data[:, :22], data[:, -11:]))
    assert data.shape[1] == args.variables
    return data
...
def read_data(error=0, is_train=True):
    j = str(error).rjust(2, '0')     # 变成01 02这种
    if is_train:
        path_fault = './data/d%s_te.dat' % j
    else:
        path_fault = './data/d%s.dat' % j

    with open(path_fault, 'r') as fr:
        data = fr.read()
    data = np.fromstring(data, dtype=np.float32, sep='   ')

    if path_fault == './data/d00.dat':
...
def construct_cnn_vae_model(block, input_shape, time_steps=args.time_steps, filters=args.filters, kersize=args.kersize, latent=args.latent):
    ...
    conv1 = Conv1D(filters=filters, kernel_size=kersize, strides=1, activation=args.activa, name='pre_conv1_{}'.format(block))(inputs)
    conv2 = Conv1D(filters=filters, kernel_size=kersize, strides=1, activation=args.activa, name='pre_conv2_{}'.format(block))(conv1)
...
def train_model(rute, vae, block, train_x, train_y, ver_x, ver_y, save_name, latent=args.latent, l1_x=args.l1_x, filter_num=args.filters, epochs=args.epochs, batch_size=args.batch_size):
    ...
    plt.show()
    print('train over')
    print(rute + '/VAE_TE_b{}_latent{}_l1{}_filter{}_{}.h5'.format(block, latent, l1_x, filter_num, save_name))
    vae.save_weights(rute + '/VAE_TE_b{}_latent{}_l1{}_filter{}_{}.h5'.format(block, latent, l1_x, filter_num, save_name))

    #  6.11 训练数据和验证数据，画出模型预测的质量变量结果
...
def test_model(rute, encoder, vae, block, train_x, fault_x, save_name, latent=args.latent, l1_x=args.l1_x, filter_num=args.filters):
    vae.load_weights(rute + '/VAE_TE_b{}_latent{}_l1{}_filter{}_{}.h5'.format(block, latent, l1_x, filter_num, save_name))
...
def contribute_plot(rute, encoder, vae, i, train_data, test_data, vars, save_name):
    axis_x = []
    axis_y = []
    for t in range(len(vars[i])):
        ...
        train_kl_loss_y, fault_kl_loss_y, _ = test_model(rute, encoder, vae, i, train_data_var, test_data_var, save_name)
